...
SmartSRE Copilot 当前是早期开源项目，仍处于开发阶段。它适合本地开发、内部评估和受控原型部署。 核心 Agent
能力、产品流程和验证覆盖完成前，公开版本 tag、GitHub 发布制品、包发布和容器镜像发布都保持禁用。
...
人类开发者请先阅读 `CONTRIBUTING.md`，其中包含分支、提交、PR、质量门、依赖、发布锁定和合并规范。

AI coding agent 还应在改动前阅读 `AGENTS.md`。

开发阶段不要创建公开版本 tag、GitHub 发布制品、包发布或容器镜像发布自动化。
